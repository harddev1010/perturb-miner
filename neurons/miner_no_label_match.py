...
def perturb(
    ...
    def verify_prefix(gsrc, order_src, k):
        ...
        return accept(_evaluate(model, clean, make_cand(sel, gsrc), target_index, device, floor, cap, skip))
    ...
    if best_soft is None:
        step_bytes = torch.zeros_like(clean_u8)
        for _ in range(_MAX_RELIN):
            if time_left() <= 3 * t_step:
                break
            x = ((clean_u8 + step_bytes).clamp(0.0, 255.0) / 255.0).view_as(clean)
            m, g = _margin_and_grad(model, x, target_index)
            gl = g.view(-1)
            if m < 0.0:  # float-flipped at this iterate -> sparsify along its (boundary) gradient
                _, order2, vc2 = _build_sparse_order(gl, clean.view(-1))
                min_flip(gl, order2, vc2, vc2)
                break
            step_bytes = -float(k_min) * gl.sign()  # dense single byte-step from clean, box-confined
    ...
    logger.info(f"[oneshot] no flip (infeasible at k={k_min}) -> clean "
                f"(k_seed={k_seed} valid_count={valid_count} m0={m0:.4f} elapsed={time.time() - t_start:.3f}s)")
    return clean.detach().clamp(0.0, 1.0)
