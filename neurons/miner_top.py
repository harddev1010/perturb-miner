...
_PROBE_DUMP_LOCK = threading.Lock()
_PROBE_DUMP_ENABLED = os.getenv("MINER_PROBE_DUMP_ENABLED", "1").strip().lower() not in (
    "0",
    "false",
    "no",
)
...
def _sanitize_probe_filename_part(value: str) -> str:
    cleaned = "".join(
...
def _probe_dump_base_name(true_label: str, task_id: str) -> str:
    return (
        f"{_sanitize_probe_filename_part(true_label)}"
        f"_{_sanitize_probe_filename_part(task_id)}"
    )


def _probe_dump_txt_path(log_dir: str, true_label: str, task_id: str) -> str:
    return os.path.join(
        log_dir,
        "probe_dumps",
        f"{_probe_dump_base_name(true_label, task_id)}.txt",
    )


def _probe_dump_png_dir(log_dir: str, true_label: str, task_id: str) -> str:
    return os.path.join(log_dir, "probe_dumps", _probe_dump_base_name(true_label, task_id))


def _write_flat_tensor_section(handle: typing.TextIO, name: str, tensor: torch.Tensor) -> None:
    flat = tensor.detach().cpu().reshape(-1)
    handle.write(f"[{name}] flat={flat.numel()}\n")
    for val in flat.tolist():
        handle.write(f"{val}\n")


def _save_chw_png(image_chw: torch.Tensor, path: str) -> None:
    os.makedirs(os.path.dirname(os.path.abspath(path)) or ".", exist_ok=True)
    clipped = image_chw.detach().cpu().clamp(0.0, 1.0)
    arr = (
        clipped.permute(1, 2, 0).mul(255.0).round().clamp(0, 255).to(torch.uint8).numpy()
    )
    Image.fromarray(arr, mode="RGB").save(path, format="PNG")


def _write_probe_dump_txt(
    path: str,
    *,
    task_id: str,
    true_label: str,
    ...
    top_classes: typing.Sequence[int],
    target_probes: typing.Sequence[dict],
) -> None:
    os.makedirs(os.path.dirname(os.path.abspath(path)) or ".", exist_ok=True)
    c, h, w = shape
    with _PROBE_DUMP_LOCK:
        with open(path, "w", encoding="utf-8") as handle:
            handle.write(f"task_id={task_id}\n")
            handle.write(f"true_label={true_label}\n")
            handle.write(f"true_idx={true_idx}\n")
            handle.write(f"shape={c}x{h}x{w}\n")
            handle.write(
                f"saved_at={datetime.datetime.now().isoformat(timespec='seconds')}\n"
            )
            _write_flat_tensor_section(handle, "grad_true", grad_true)
            for probe in target_probes:
                handle.write(
                    f"\n=== task_id={task_id} target={probe['target']} ===\n"
                )
                handle.write(f"actual_reduction={probe['actual_reduction']:.8e}\n")
                handle.write(f"predicted_reduction={probe['predicted_reduction']:.8e}\n")
                handle.write("gap_reductions:\n")
                for cls in top_classes:
                    handle.write(
                        f"{cls}:{probe['gap_reductions'][cls]:.8e}\n"
                    )
                _write_flat_tensor_section(handle, "grad_t_raw", probe["grad_t_raw"])


def _flush_probe_dumps(
    log_dir: str,
    *,
    task_id: str,
    true_label: str,
    ...
    grad_true: torch.Tensor,
    top_classes: typing.Sequence[int],
    challenge_clean: torch.Tensor,
    target_probes: typing.Sequence[dict],
) -> None:
    if not target_probes:
        return

    txt_path = _probe_dump_txt_path(log_dir, true_label, task_id)
    png_dir = _probe_dump_png_dir(log_dir, true_label, task_id)

    _write_probe_dump_txt(
        txt_path,
        task_id=task_id,
    ...
    with _PROBE_DUMP_LOCK:
        _save_chw_png(challenge_clean, os.path.join(png_dir, "clean.png"))
        for probe in target_probes:
            probe_name = f"target{probe['target']}_probe.png"
            _save_chw_png(probe["probe_image"], os.path.join(png_dir, probe_name))

    logger.info(
        f"[PROBE_DUMP] task_id={task_id} saved txt={txt_path} "
        f"png_dir={png_dir} targets={len(target_probes)}"
    )
...
_MAX_COARSE_PROBES = int(os.getenv("MINER_MAX_COARSE_PROBES", "6"))
...
# Minimum marginal_ratio when stepping from anchor batch k0 to a larger k:
#   marginal_gain = (drop_k - drop_k0) / (k - k0)
#   marginal_ratio = marginal_gain / (drop_k0 / k0)
# If marginal_ratio falls below this, do not select the larger batch; refine
# inside (k0, k) to find the largest k that still meets the ratio.
_TRUST_REGION_COLLAPSE_RATIO = float(os.getenv("MINER_TRUST_REGION_COLLAPSE_RATIO", "0.8"))

# Hard-flip gate: true logit must be <= this value (negative) below the winner.
# Env may be set with positive magnitude by mistake; always normalize to negative.
_flip_margin_raw = float(os.getenv("MINER_FLIP_MARGIN_EPS", "-0.015"))
if _flip_margin_raw > 0.0:
    pylogging.getLogger(__name__).warning(
        "MINER_FLIP_MARGIN_EPS=%s is positive; using -%s (margin must be "
        "negative below the decision boundary)",
        _flip_margin_raw,
        abs(_flip_margin_raw),
    )
_FLIP_MARGIN_EPS = -abs(_flip_margin_raw)
# Continue safety-grow after first hard flip until margin reaches this depth.
_safety_grow_raw = float(os.getenv("MINER_SAFETY_GROW_TARGET", "-0.02"))
_SAFETY_GROW_TARGET = -abs(_safety_grow_raw)
if _SAFETY_GROW_TARGET > _FLIP_MARGIN_EPS:
    _SAFETY_GROW_TARGET = _FLIP_MARGIN_EPS
...
# ── Current-adv target efficiency rescore ────────────────────────────────
_TARGET_RESCORE_ENABLED = os.getenv("MINER_TARGET_RESCORE_ENABLED", "1").strip() not in ("0", "false", "no")

# Re-score target classes every N grow loops. Do not do this every loop unless you have enough time.
_TARGET_RESCORE_EVERY = int(os.getenv("MINER_TARGET_RESCORE_EVERY", "4"))

# Number of current top wrong classes to check. 20-30 is usually better than 100 for runtime.
_TARGET_RESCORE_TOPK = int(os.getenv("MINER_TARGET_RESCORE_TOPK", "30"))

# Switch only if new target is clearly better. 0.72 means at least 28% lower estimated remaining k.
_TARGET_SWITCH_K_RATIO = float(os.getenv("MINER_TARGET_SWITCH_K_RATIO", "0.72"))

# Do emergency target rescore if selected pixels exceed this multiple of k_est and gap is still large.
_TARGET_RESCORE_K_MULT = float(os.getenv("MINER_TARGET_RESCORE_K_MULT", "1.75"))

# Avoid expensive rescoring when almost out of time.
_TARGET_RESCORE_MIN_TIME = float(os.getenv("MINER_TARGET_RESCORE_MIN_TIME", "0.85"))
...
def _quality_on_png(
    ...
    q = _measure_adv_quality(model, clean, decoded, true_label)
    return decoded, q
...
class PerturbMiner:
    def __init__(self, config: typing.Any) -> None:
        ...
        self._probe_dump_dir = getattr(
            getattr(config, "logging", None), "logging_dir", "./logs"
        )
    ...
    def _attack(
        ...
        challenge_clean = (
            (clean.detach() * 255.0).round().clamp(0, 255).float() / 255.0
        ).cpu()
        target_probes: list[dict] = []
        ...
        for i, t in enumerate(top_classes):
            ...
            logger.info(f"[TARGET] top_mean={top_mean:.2e} top_sum={top_sum:.2e}")
            ...
            # Empirical probe: perturb top-1000 valid pixels by +/-1/255 along g_t
            # and measure how much each top-class margin actually closes.
            base_u8 = (clean.detach() * 255.0).round().clamp(0, 255).long()
            flat_base = base_u8.reshape(-1)

            signs_probe = (-g_t.sign()).long()
            valid_step = (
                ((signs_probe > 0) & (flat_base < 255)) |
                ((signs_probe < 0) & (flat_base > 0))
            )
            valid_count = int(valid_step.sum().item())
            if valid_count <= 0:
                continue

            abs_g_probe = abs_g.clone()
            abs_g_probe[~valid_step] = 0.0
            n_probe = min(1000, valid_count)
            probe_idx = abs_g_probe.topk(n_probe).indices

            flat_probe = flat_base.clone()
            flat_probe[probe_idx] = (
                flat_probe[probe_idx] + signs_probe[probe_idx]
            ).clamp(0, 255)

            probe_img = flat_probe.reshape(C, H, W).float() / 255.0

            with torch.no_grad():
                lg_probe = logits_for_images(
                    self.model, probe_img.unsqueeze(0)
                ).squeeze(0).float()

            true_logit_clean = float(true_logit.item())
            true_logit_probe = float(lg_probe[true_idx].item())
            gap_reductions: dict[int, float] = {}
            for c in top_classes:
                gap_clean_c = true_logit_clean - float(lg[c].item())
                gap_pert_c = true_logit_probe - float(lg_probe[c].item())
                gap_reductions[c] = gap_clean_c - gap_pert_c

            actual_reduction_t = float(gap_reductions[t])
            predicted_reduction = float((eps * abs_g[probe_idx].sum()).item())

            logger.info(
                f"[PROBE1000] target={t} n_probe={n_probe} valid={valid_count} "
                f"actual_reduction={actual_reduction_t:.4e} "
                f"predicted_reduction={predicted_reduction:.4e} "
                f"ratio={actual_reduction_t / max(predicted_reduction, 1e-10):.3f}"
            )
            log_attack(
                f"[PROBE1000] target={t} actual={actual_reduction_t:.4e} "
                f"predicted={predicted_reduction:.4e}"
            )
            reduction_parts = [
                f"{c}:{gap_reductions[c]:.3e}" for c in top_classes
            ]
            logger.info(
                f"[PROBE1000] target={t} gap_reductions_all="
                + "{" + ", ".join(reduction_parts) + "}"
            )

            if _PROBE_DUMP_ENABLED:
                target_probes.append({
                    "target": t,
                    "actual_reduction": actual_reduction_t,
                    "predicted_reduction": predicted_reduction,
                    "gap_reductions": gap_reductions,
                    "grad_t_raw": grad_t_raw.detach().cpu(),
                    "probe_image": probe_img.detach().cpu(),
                })
            ...
            targets.append({
                ...
                "k_estimated":    k_est,
                "gradient":       g_t,
                "actual_gap_reduction": actual_reduction_t,
                "predicted_gap_reduction": predicted_reduction,
                "gap_reductions": gap_reductions,
            }) 
        if not targets:
        ...
        def choose_batch_by_marginal_probe(
            *,
            ...
            def _marginal_ratio(anchor: dict, probe: dict) -> float:
                """Marginal efficiency of probe k vs anchor k0, relative to anchor avg eff."""
                dk = max(int(probe["k"]) - int(anchor["k"]), 1)
                marginal_gain = (float(probe["drop"]) - float(anchor["drop"])) / dk
                prev_eff = max(float(anchor["eff"]), 1e-12)
                return marginal_gain / prev_eff
            ...
            def _probe_candidates(candidates: list[int]) -> tuple[list[dict], torch.Tensor]:
                ...
                Stops early once marginal_ratio from the last good probe falls below
                _TRUST_REGION_COLLAPSE_RATIO — larger batches are unlikely to recover.
                """
                ...
                results = []
                good_prev: typing.Optional[dict] = None
                ...
                for k in candidates:
                    ...
                    if actual_drop > 1e-8:
                        if good_prev is not None:
                            ratio = _marginal_ratio(good_prev, row)
                            if ratio < _TRUST_REGION_COLLAPSE_RATIO:
                                skipped = len(candidates) - len(results)
                                if skipped > 0:
                                    logger.info(
                                        f"[PROBE_EARLY_STOP] k={k} "
                                        f"marginal_ratio={ratio:.3f} "
                                        f"< {_TRUST_REGION_COLLAPSE_RATIO:.3f} "
                                        f"skipped={skipped}"
                                    )
                                break
                        good_prev = row
            ...
            def _efficiency_ceiling_k(
                results: list[dict],
                last_good: dict,
                first_weak: typing.Optional[dict],
            ) -> int:
                """Max k allowed for flip / batch selection within the 0.8 chain."""
                if first_weak is None:
                    positive = [r for r in results if r["drop"] > 1e-8]
                    if positive:
                        return int(max(positive, key=lambda r: int(r["k"]))["k"])
                    return int(last_good["k"])
                boundary = _largest_at_marginal_boundary(results, last_good, first_weak)
                return int(boundary["k"])

            def _is_in_efficiency_range(
                r: dict,
                last_good: dict,
                ceiling_k: int,
            ) -> bool:
                k = int(r["k"])
                anchor_k = int(last_good["k"])
                if k > ceiling_k:
                    return False
                if k <= anchor_k:
                    return True
                if "drop" not in r:
                    # Fine-search probes (_probe_one_k) omit drop; hi is already
                    # capped at ceiling_k so the trust region is respected.
                    return True
                return _marginal_ratio(last_good, r) >= _TRUST_REGION_COLLAPSE_RATIO

            def _find_efficiency_valid_flips(
                results: list[dict],
            ) -> tuple[list[dict], typing.Optional[dict], int]:
                """
                Flips that lie inside the marginal-ratio trust region.

                Ignores flips beyond the efficiency ceiling — e.g. 1024 flips when
                256→512 already failed the 0.8 ratio.
                """
                positive = sorted(
                    [r for r in results if r["drop"] > 1e-8],
                ...
                ceiling_k = _efficiency_ceiling_k(results, last_good, first_weak)
                valid = [
                    r for r in results
                    if (r.get("flipped") or r.get("gap", 0) < 0.0)
                    and _is_in_efficiency_range(r, last_good, ceiling_k)
                ]
                return valid, last_good, ceiling_k
            ...
            def _fine_search_first_flip(
                *,
                ...
                """
                Smallest flipping k inside the efficiency-valid range.

                Example: coarse flip at 256 with 128→256 passing 0.8 searches
                (128, 256] for the minimum flipping batch. Flips above ceiling_k
                (e.g. 1024 when 256→512 collapsed) are never considered.
                """
                hi = min(int(chosen_flip["k"]), int(ceiling_k))

                lower_nonflips = [
                    int(r["k"])
                    for r in results
                    if int(r["k"]) < hi
                    and not (r.get("flipped") or r.get("gap", 0) < 0.0)
                    and _is_in_efficiency_range(r, last_good, ceiling_k)
                ]
                ...
                while lo <= hi and (grow_deadline - time.perf_counter()) >= 0.18:
                    ...
                    if (
                        r_mid.get("flipped", False)
                        and _is_in_efficiency_range(r_mid, last_good, ceiling_k)
                    ):
                        best_k = mid
                        hi = mid - 1
                    else:
            ...
            def _try_efficiency_valid_flip(
                results: list[dict],
                top_pool: torch.Tensor,
                *,
                log_tag: str,
            ) -> typing.Optional[tuple[int, torch.Tensor]]:
                """
                If any flip lies inside the 0.8 trust region, fine-search the
                smallest such k and return (fine_k, top_pool_slice). Else None.
                """
                valid_flips, last_good, ceiling_k = _find_efficiency_valid_flips(results)
                if not valid_flips or last_good is None:
                    raw_flips = [
                        r for r in results
                        if r.get("flipped") or r.get("gap", 0) < 0.0
                    ]
                    if raw_flips:
                        ignored = [
                            int(r["k"]) for r in raw_flips
                            if not _is_in_efficiency_range(r, last_good or {"k": 0}, ceiling_k)
                        ] if last_good is not None else [int(r["k"]) for r in raw_flips]
                        if ignored:
                            logger.info(
                                f"[{log_tag}] ignoring out-of-range flips k={ignored} "
                                f"ceiling={ceiling_k} anchor={last_good['k'] if last_good else '?'}"
                            )
                    return None

                chosen = min(valid_flips, key=lambda r: int(r["k"]))
                ...
                logger.info(
                    f"[{log_tag}] anchor={last_good['k']} ceiling={ceiling_k} "
                    f"coarse={[(r['k'], round(r['gap'],5), round(r['drop'],5)) for r in results if r.get('drop') is not None]} "
            ...
            def _merge_probe_results(
                ...
                by_k = {int(r["k"]): r for r in existing}
                for r in new_rows:
                    by_k[int(r["k"])] = r
                return sorted(by_k.values(), key=lambda r: int(r["k"]))

            def _find_collapse_pair(results: list[dict]) -> tuple[typing.Optional[dict], typing.Optional[dict]]:
                """
                Return (last_good, first_weak) when marginal_ratio drops below threshold.

                Walk coarse doubling probes in k order. last_good is the largest
                batch whose step from the previous good batch still meets the ratio.
                first_weak is the first batch that fails.
                """
                positive = sorted(
                    [r for r in results if r["drop"] > 1e-8],
                    key=lambda r: int(r["k"]),
                ...
                for r in positive[1:]:
                    if _marginal_ratio(good_prev, r) < _TRUST_REGION_COLLAPSE_RATIO:
                        return good_prev, r
                    good_prev = r
            ...
            def _largest_candidate_still_good(results: list[dict]) -> bool:
                """
                Return True only when every coarse-doubling step still meets the
                marginal-ratio threshold — i.e. no collapse anywhere in the chain.

                Example: [64, 128, 256, 512, 1024] with 256→512 failing must
                return False even if 512→1024 passes; expansion stops at collapse.
                """
                positive = sorted(
                    [r for r in results if r["drop"] > 1e-8],
                    key=lambda r: int(r["k"]),
                ...
                _, first_weak = _find_collapse_pair(positive)
                return first_weak is None

            def _largest_at_marginal_boundary(
                results: list[dict],
                anchor: dict,
                first_weak: dict,
            ) -> dict:
                """
                Largest k in (anchor, first_weak) with marginal_ratio(anchor, k) >= threshold.
                """
                positive = sorted(
                    [r for r in results if r["drop"] > 1e-8],
                    key=lambda r: int(r["k"]),
                ...
                for r in positive:
                    ...
                    if k >= weak_k:
                        break
                    if _marginal_ratio(anchor, r) >= _TRUST_REGION_COLLAPSE_RATIO and k > int(best["k"]):
                        best = r

                return best

            def _refine_marginal_boundary(
                *,
                top_pool: torch.Tensor,
                anchor: dict,
                first_weak: dict,
                ...
                """
                Binary-search the largest k in (anchor, first_weak) that keeps
                marginal_ratio(anchor, k) >= threshold.
                """
                lo = int(anchor["k"])
                hi = int(first_weak["k"]) - 1
                best = anchor
                merged = _merge_probe_results(results, [])

                if hi <= lo:
                    return best, top_pool, merged

                known = {int(r["k"]): r for r in merged}

                while hi - lo > 4 and (grow_deadline - time.perf_counter()) >= 0.18:
                    mid = (lo + hi + 1) // 2
                    if mid not in known:
                        new_rows, top_pool = _probe_candidates([mid])
                        merged = _merge_probe_results(merged, new_rows)
                        known = {int(r["k"]): r for r in merged}
                        r_mid = known[mid]
                    else:
                        r_mid = known[mid]

                    if _marginal_ratio(anchor, r_mid) >= _TRUST_REGION_COLLAPSE_RATIO:
                        lo = mid
                        best = r_mid
                    else:
                        hi = mid - 1

                if lo in known and int(known[lo]["k"]) > int(best["k"]):
                    best = known[lo]
            ...
            def _choose_from_results(results: list[dict]) -> typing.Optional[dict]:
                """
                Choose smallest efficiency-valid flip, otherwise the largest batch
                whose marginal_ratio from the last good coarse anchor still meets
                the threshold (0.8 by default).
                """
                if not results:
                    return None

                valid_flips, last_good, ceiling_k = _find_efficiency_valid_flips(results)
                if valid_flips and last_good is not None:
                    return min(valid_flips, key=lambda r: int(r["k"]))
                ...
                return _largest_at_marginal_boundary(positive, last_good, first_weak)
            ...
            while (
                ...
                and not _find_efficiency_valid_flips(results)[0]
            ):
                ...
                new_rows, top_pool = _probe_candidates([next_k])
                results = _merge_probe_results(results, new_rows)

                # Refresh time guard after each expansion.
                time_left = grow_deadline - time.perf_counter()

            # 2. Efficiency-valid flip: smallest k inside the 0.8 trust region.
            flip_pick = _try_efficiency_valid_flip(
                results, top_pool, log_tag="BATCH_PROBE_FLIP_FINE",
            )
            if flip_pick is not None:
                fine_k, top_slice = flip_pick
                return int(fine_k), top_slice

            # 3. Marginal-ratio boundary refinement between last_good and first_weak.
            ...
            if (
                ...
                boundary_result, top_pool, results = _refine_marginal_boundary(
                    top_pool=top_pool,
                    anchor=last_good,
                ...
                logger.info(
                    f"[BATCH_REFINE_MARGINAL] anchor={last_good['k']} weak={first_weak['k']} "
                    f"boundary={boundary_result['k']} "
                    f"ratio={_marginal_ratio(last_good, boundary_result):.3f} "
                    f"threshold={_TRUST_REGION_COLLAPSE_RATIO:.3f}"
                )

                flip_pick = _try_efficiency_valid_flip(
                    results, top_pool, log_tag="BATCH_REFINE_FLIP_FINE",
                )
                if flip_pick is not None:
                    fine_k, top_slice = flip_pick
                    return int(fine_k), top_slice
            ...
            chosen = boundary_result if boundary_result is not None else _choose_from_results(results)
            ...
            logger.info(
                ...
                f"boundary={None if boundary_result is None else boundary_result['k']} "
        ...
        def _safety_margin_reached(margin: float) -> bool:
            return margin <= _SAFETY_GROW_TARGET
        ...
        def _record_candidate(
            ...
            *,
            ...
            flip_q = _measure_adv_quality(
                self.model, clean, snapped, true_idx
            )
            cand = {
                "image":    snapped.clone(),
                "k":        len(sel_snapshot),
        ...
        ELIM_RESERVE = float(os.getenv("MINER_ELIM_RESERVE", "0.5"))
        FINAL_BUFFER = 0.25
        ...
        grow_iter = 0
        target_rescore_cooldown = 0
        current_target_k_est = float(k_est)

        def _rescore_targets_from_current_adv(
            *,
            x_in: torch.Tensor,
            lg_in: torch.Tensor,
            ...
            """
            Re-score candidate target classes from current_adv.

            Purpose:
            - Check whether current target is still efficient.
            - Estimate remaining pixel cost using only currently available pixels.
            - Return best target candidate, or None if no useful candidate exists.

            Important:
            - Starts from current_adv, not clean.
            - Uses valid pixels only: pixels still equal to clean and movable by +/-1/255.
            - This is expensive, so call only every few grow loops or when stuck.
            """
            nonlocal current_target_k_est

            if grow_deadline - time.perf_counter() < _TARGET_RESCORE_MIN_TIME:
                return None

            # Current top wrong classes by logit.
            with torch.no_grad():
                sorted_classes = lg_in.detach().argsort(descending=True).tolist()
            ...
            for c in sorted_classes:
                ...
                if len(candidate_classes) >= int(max_targets):
                    break

            # Always include old target and current runner-up.
            candidate_classes.extend([int(current_target_idx), int(runner_up_idx)])

            # Also include the original pre-ranked targets if available.
            # This protects against a class no longer being top-logit but still having strong sparse gradient.
            try:
                candidate_classes.extend([int(t["target"]) for t in targets[:10]])
            except Exception:
                pass
            ...
            # True gradient at current_adv.
            if x_in.grad is not None:
                x_in.grad.zero_()

            lg_in[true_idx].backward(retain_graph=True)
            grad_true_cur = x_in.grad.detach().reshape(-1).clone()
            ...
            unused = (q_cur == q_clean)

            rescored: list[dict] = []

            for i, t in enumerate(candidate_classes):
                if int(t) == int(true_idx):
                    continue

                if x_in.grad is not None:
                    x_in.grad.zero_()

                retain = i < (len(candidate_classes) - 1)
                try:
                    lg_in[t].backward(retain_graph=retain)
                except RuntimeError:
                    # If graph was unexpectedly freed, skip safely.
                    logger.warning("[TARGET_RESCORE] backward graph unavailable; skip rescore")
                    return None

                grad_t_cur = x_in.grad.detach().reshape(-1).clone()

                g_boundary = grad_true_cur - grad_t_cur
                margin_t = float(lg_in[true_idx].item() - lg_in[t].item())

                if margin_t <= _FLIP_MARGIN_EPS:
                    # Already hard enough against this target.
                    k_est_t = 1.0
                    sparse_score_t = -1.0
                    top_mean = 0.0
                    top_sum = 0.0
                    valid_count = int(unused.sum().item())
                else:
                    g_norm2 = float(g_boundary.norm(2).item())
                    if g_norm2 < 1e-10:
                        continue

                    # Direction for reducing true-target margin.
                    signs_t = -g_boundary.sign()
                    ...
                    valid_t = unused & valid_step_t
                    valid_count = int(valid_t.sum().item())

                    if valid_count <= 0:
                        continue

                    abs_g = g_boundary.abs()
                    abs_g[~valid_t] = 0.0

                    n_top = min(max(100, int(N * 0.005)), valid_count)
                    if n_top <= 0:
                        continue

                    top_vals = abs_g.topk(n_top).values
                    top_sum = float(top_vals.sum().item())
                    top_mean = float(top_vals.mean().item())

                    if top_mean <= 1e-12 or top_sum <= 1e-12:
                        continue

                    # Feasibility using only currently available pixels.
                    if (eps * top_sum) < margin_t * 0.05:
                        continue

                    # Estimated remaining changed pixels.
                    k_est_t = margin_t / max(eps * top_mean, 1e-10)

                    # Sparse score, lower is better.
                    sparse_score_t = margin_t / max(eps * top_sum, 1e-10)

                rescored.append({
                    "target": int(t),
                    "margin": float(margin_t),
                    "k_est": float(k_est_t),
                    "sparse_score": float(sparse_score_t),
                    "top_mean": float(top_mean),
                    "top_sum": float(top_sum),
                    "valid_count": int(valid_count),
                    "gradient": g_boundary.detach().clone(),
                })

            if not rescored:
                return None

            # Prefer lowest estimated remaining pixels.
            rescored.sort(key=lambda r: (float(r["k_est"]), float(r["sparse_score"])))

            best = rescored[0]
            cur_rows = [r for r in rescored if int(r["target"]) == int(current_target_idx)]
            cur = cur_rows[0] if cur_rows else None

            if cur is not None:
                current_target_k_est = float(cur["k_est"])

            logger.info(
                "[TARGET_RESCORE] "
                f"current={current_target_idx} "
                f"cur_k={None if cur is None else round(float(cur['k_est']), 1)} "
                f"cur_margin={None if cur is None else round(float(cur['margin']), 4)} "
                f"best={best['target']} "
                f"best_k={float(best['k_est']):.1f} "
                f"best_margin={float(best['margin']):.4f} "
                f"top5={[(r['target'], round(float(r['k_est']), 1), round(float(r['margin']), 4)) for r in rescored[:5]]}"
            )

            log_attack(
                f"[TARGET_RESCORE] current={current_target_idx} "
                f"best={best['target']} best_k={float(best['k_est']):.1f} "
                f"best_margin={float(best['margin']):.4f}"
            )
        ...
        while True:
            ...
            # 1. Cheap switch: follow runner-up if it is clearly closer.
            # 2. Expensive switch: periodically rescore targets from current_adv
            #    using current gradients and currently available pixels.
            ...
            if runner_up_idx != target_idx and rank2_gap + 0.02 < target_gap:
                ...
                logger.info(
                    f"[TARGET_SWITCH_RUNNER_UP] old={old_target_idx} new={target_idx} "
                ...
                near_boundary_steps = 0
                target_rescore_cooldown = 2
                switched_target = True

            else:
                cur_gap = min(target_gap, rank2_gap)

            # Expensive current_adv target rescore.
            # Use this when current target may be inefficient.
            grow_iter += 1

            should_rescore_target = (
                _TARGET_RESCORE_ENABLED
                and not has_soft_flip
                and attack_phase == "grow"
                and target_rescore_cooldown <= 0
                and (grow_deadline - time.perf_counter()) >= _TARGET_RESCORE_MIN_TIME
                and (
                    grow_iter % max(1, _TARGET_RESCORE_EVERY) == 0
                    or (
                        len(selected) > int(max(300, k_est * _TARGET_RESCORE_K_MULT))
                        and cur_gap > 0.50
                    )
                )
            )

            if should_rescore_target:
                rescore = _rescore_targets_from_current_adv(
                    x_in=x_in,
                    lg_in=lg_in,
                    current_target_idx=target_idx,
                    runner_up_idx=runner_up_idx,
                    max_targets=_TARGET_RESCORE_TOPK,
                )

                if rescore is not None:
                    best_rescored = rescore["best"]
                    current_rescored = rescore["current"]

                    # Switch when:
                    # - no current target score exists, or
                    # - new target has clearly lower estimated remaining k.
                    current_k_now = (
                        float(current_rescored["k_est"])
                        if current_rescored is not None
                        else float(current_target_k_est)
                    )
                    best_k_now = float(best_rescored["k_est"])

                    if (
                        int(best_rescored["target"]) != int(target_idx)
                        and best_k_now < current_k_now * _TARGET_SWITCH_K_RATIO
                    ):
                        old_target_idx = target_idx
                        old_k = current_k_now

                        target_idx = int(best_rescored["target"])
                        target_gap = float(best_rescored["margin"])
                        cur_gap = min(target_gap, rank2_gap)

                        g_t = best_rescored["gradient"]
                        g_norm1 = g_t.abs().sum().item()

                        # Update k_est to remaining estimate from current_adv.
                        k_est = max(int(best_k_now * 1.15), len(selected) + 100, 300)
                        current_target_k_est = float(best_k_now)
                        ...
                        logger.info(
                            f"[TARGET_SWITCH_RESCORING] old={old_target_idx} new={target_idx} "
                            f"old_k={old_k:.1f} new_k={best_k_now:.1f} "
                            f"ratio={best_k_now / max(old_k, 1e-9):.3f} "
                            f"target_gap={target_gap:.4f} rank2_gap={rank2_gap:.4f} "
                            f"k_est_updated={k_est}"
                        )
                        log_attack(
                            f"[TARGET_SWITCH_RESCORING] old={old_target_idx} new={target_idx} "
                            f"old_k={old_k:.1f} new_k={best_k_now:.1f} gap={cur_gap:.4f} "
                            f"k_est={k_est}"
                        )

            if target_rescore_cooldown > 0:
                target_rescore_cooldown -= 1
            ...
            # Two backward passes for the finally selected target.
            # Rebuild graph here because target rescoring may have consumed lg_in's graph.
            x_grad = current_adv.detach().requires_grad_(True)
            lg_grad = logits_for_images(self.model, x_grad.unsqueeze(0)).squeeze(0)

            if x_grad.grad is not None:
                x_grad.grad.zero_()

            lg_grad[true_idx].backward(retain_graph=True)
            g_true_cur = x_grad.grad.detach().reshape(-1).clone()

            x_grad.grad.zero_()
            lg_grad[target_idx].backward()
            g_tgt_cur = x_grad.grad.detach().reshape(-1).clone()
            ...
            if cur_gap < 2.0 or rank2_gap < 0.05:
                ...
                if _flipped:
                    ...
                    _record_candidate(
                        ...
                        tag="CHECK", selected_snapshot=selected.copy(),
                    )
                    if _safety_margin_reached(_margin) and best_hard_result is not None:
                        logger.info(
                            f"[SAFETY_GROW_DONE] k={len(selected)} "
                            f"margin={_margin:.6f} target={_SAFETY_GROW_TARGET:.6f} "
                            f"— stopping growth"
                        )
                        break
            ...
            if _flipped2:
                ...
                if _safety_margin_reached(_margin2):
                    logger.info(
                        f"[SAFETY_GROW_DONE] k={len(selected)} margin={_margin2:.6f} "
                        f"target={_SAFETY_GROW_TARGET:.6f} — stop growth and start elimination"
                    )
                    log_attack(
                        f"[SAFETY_GROW_DONE] k={len(selected)} margin={_margin2:.6f}"
                    )
                ...
                if _is_hard_flip(_margin2):
                    logger.info(
                        f"[HARD_FLIP_WEAK] k={len(selected)} margin={_margin2:.6f} "
                        f"hard={_FLIP_MARGIN_EPS:.6f} target={_SAFETY_GROW_TARGET:.6f} "
                        f"— continue safety grow"
                    )
                    continue

                logger.info(
                    f"[SOFT_FLIP_GROW] k={len(selected)} margin={_margin2:.6f} "
                    f"needs hard<={_FLIP_MARGIN_EPS:.6f} safety<={_SAFETY_GROW_TARGET:.6f}"
                )
                continue
        ...
        if best_result is None and len(selected) > 0:
            ...
            _record_candidate(
                ...
                tag="LATE", selected_snapshot=selected.copy(),
            )
            best_result = best_hard_result
        ...
        _, final_q = _quality_on_png(self.model, clean, curr, true_idx)
        ...
        if _PROBE_DUMP_ENABLED and target_probes:
            result["probe_dump"] = {
                "task_id": task_id,
                "true_label": true_label,
                "true_idx": true_idx,
                "shape": (C, H, W),
                "grad_true": grad_true.detach().cpu(),
    ...
    async def forward(self, synapse: AttackChallenge) -> AttackChallenge:
        ...
        try:
            ...
            _, submit_q = _quality_on_png(
                self.model, clean, result["image"], true_idx
            )
            ...
            probe_dump = result.get("probe_dump")
            if _PROBE_DUMP_ENABLED and probe_dump:
                _flush_probe_dumps(self._probe_dump_dir, **probe_dump)
            return synapse
        finally:
            attack_log.flush()
