...
def perturb(
    ...
    # Trust the validator's epsilon as the ceiling. We don't know the validator's internal
    # max_linf_delta, so imposing our own guess can either over-restrict (missing valid flips)
    # or under-restrict (if the validator is tighter). epsilon is the explicit per-challenge
    # upper bound the validator chose to send; stay within it.
    cap = float(epsilon)
    ...
    # (disabled: FGSM probes short-circuit to k=1 on every image for non-robust models,
    # masking whether Phases 2-5 ever run. Commented out to force the full search path.)
    # for k in levels[:3]:
    ...
    # =========================== PHASE 5 — margin deepening ================================
    # Use remaining budget to push confidence further from the decision boundary at the same
    # committed k level. Only accepts candidates that are still valid AND have a more negative
    # margin (logit[true] - logit[best_other] more negative => higher adversarial confidence).
    if best is not None and time_left() > t_step + 1.3 * t_png:
        r = float(best["linf"])
        alpha = max(_Q, r / 4.0)
        delta = (best["cand"] - clean).clamp(-r, r)
        best_margin, _ = _margin_and_grad(model, best["cand"], target_index)
        while time_left() > t_step + 1.3 * t_png:
            x = (clean + delta).clamp(0.0, 1.0)
            margin, grad = _margin_and_grad(model, x, target_index)
            if margin < best_margin:
                c0 = time.time()
                res = _evaluate(model, clean, x, target_index, device, floor, cap)
                t_png = max(1e-4, time.time() - c0)
                if res["valid"]:
                    best_margin = margin
                    best = res
            delta = ((x - alpha * grad.sign()).clamp(0.0, 1.0) - clean).clamp(-r, r)
...
class PerturbMiner:
    ...
    async def forward(self, synapse: AttackChallenge) -> AttackChallenge:
        return await self._forward_impl(synapse)

    async def _forward_impl(self, synapse: AttackChallenge) -> AttackChallenge:
        ...
        try:
            ...
            # Log the REAL post-PNG scores the validator will measure.
            seen = decode_image_b64(synapse.perturbed_image_b64)
            seen_dev = seen.to(self.device)
            diff = seen_dev - clean
            linf = float(diff.abs().max().item())
            rmse = float(torch.sqrt(torch.mean(diff * diff)).item())
            logger.info(
                f"Finished task={getattr(synapse, 'task_id', 'unknown')} target_idx={target_index} "
                f"linf={linf:.6f} rmse={rmse:.6f} min_delta={min_delta:.6f} "
                f"epsilon={epsilon:.4f} timeout={timeout_seconds:.1f}s elapsed={time.time() - t_received:.3f}s"
            )
            # Verify submitted image: run model on clean and adv to confirm label shift.
            # Diagnoses edge cases like clean-already-misclassified (clean_correct=False means
            # target_index doesn't match model's actual clean prediction -> trivial "flip").
            with torch.no_grad():
                clean_logits = logits_for_images(model=self.model, image_bchw=clean.unsqueeze(0))[0]
                clean_probs = torch.softmax(clean_logits, dim=0)
                clean_pred = int(clean_probs.argmax().item())
                clean_conf = float(clean_probs[clean_pred].item())

                adv_logits = logits_for_images(model=self.model, image_bchw=seen_dev.unsqueeze(0))[0]
                adv_probs = torch.softmax(adv_logits, dim=0)
                adv_pred = int(adv_probs.argmax().item())
                adv_conf = float(adv_probs[adv_pred].item())
                true_conf = float(adv_probs[target_index].item())

            logger.info(
                f"Verify task={getattr(synapse, 'task_id', 'unknown')} "
                f"true_label={getattr(synapse, 'true_label', None)} target_idx={target_index} "
                f"clean_pred={clean_pred}(conf={clean_conf:.3f}) clean_correct={clean_pred == target_index} "
                f"adv_pred={adv_pred}(conf={adv_conf:.3f}) true_conf_adv={true_conf:.3f} "
                f"label_flipped={adv_pred != target_index}"
            )
        except Exception as err:
