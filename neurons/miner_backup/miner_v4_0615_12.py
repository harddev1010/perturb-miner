"""
miner_v4_0615_12.py — BACKUP of neurons/miner.py as of 2026-06-15

APPROACH: Quantization-aware sparse binary search (Phase 1) + FMN-LInf (Phase 2) + PGD (Phase 3).

Phase 1 — Sparse top-k binary search (main workhorse):
  • Sort all pixel channels by |gradient| descending (clamping-aware).
  • Estimate n_base: linear threshold = m0 / (k_min/255) gives the cumsum crossing point.
  • Exponential doubling from n_base to find first upper bound that flips (O(log N) evals).
  • Binary search in [last_fail, first_flip] to minimize the channel count (O(log N) evals).
  • Perturbation: move top-n channels by exactly k_min/255 against their gradient sign.
  • PNG round-trip verified: every candidate is encoded→decoded before checking the label.

Phase 2 — FMN-LInf direction (foolbox, optional):
  • Runs foolbox LInfFMNAttack to find a continuous minimum-norm direction.
  • Scales that direction to each discrete k/255 level and verifies each one.
  • Skipped if Phase 1 already achieved k_min or time is too tight.

Phase 3 — PGD fallback (reliability net):
  • Only runs if Phase 1 and 2 both found nothing.
  • Multi-step projected gradient descent for increasing k.
...
Scoring (validator mirror):
  ...
  effective_max = min(epsilon, 0.03)   [SPEED_WEIGHT=0 so speed is irrelevant]

Reserve: 3.0s (default) — headroom for PNG encode + network + final verify.

Improvement ideas (implemented in miner_v5_* variants):
  1. Target class selection: probe N closest wrong classes, attack toward easiest (fewest pixels).
  2. Re-linearize at boundary: after first flip found, bisect to boundary, recompute gradient there.
  3. FMN-L0 phase: use foolbox L0FMNAttack + snap to ±1/255 as an alternative sparse solution.
"""
