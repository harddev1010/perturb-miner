...
MATH (CW margin m = logit[true] - max_{j!=true} logit[j], step q = 1/255):
  ...
  HYBRID MARGIN (PERTURB_SOFT_GRAD): the chunk's ranking + step DIRECTION can instead come from a
  soft objective m_soft = z_true - tau·logsumexp(top-M wrong / tau), blending the top-M competing
  classes for a steadier direction; the HARD margin above still decides accept/break/flip.
...
TUNABLE ENV VARS:
  ...
  PERTURB_LOG_TF32_GAP          (0)    log tf32_gap=m_on-m_off on the final candidate (tuning data
                                       for KAPPA_RESID); free with the envelope on, else +1 forward
  PERTURB_SOFT_GRAD             (0)    rank/step from a soft top-M objective; HARD margin still
                                       judges accept/break (PERTURB_SOFT_TOP_M/TAU/TAU_MED/GAP_LO/
                                       GAP_HI/NEAR_THRESH/NEAR_M/NEAR_TAU/NEAR_SHRINK tune it)
...
# Measure the TF32 drift gap (m_on - m_off) on the final candidate for tuning PERTURB_KAPPA_RESID
# from real numbers. Free when the envelope is on (m_on already computed); else one extra forward at
# finalize (CUDA only). Logged as tf32_gap=; does not change the accept decision.
_LOG_TF32_GAP = os.getenv("PERTURB_LOG_TF32_GAP", "0").strip().lower() in {"1", "true", "yes", "on"}
...
_CHUNK_ALPHA = _env_float("PERTURB_CHUNK_ALPHA", 0.3)
_CHUNK_P_STEP = _env_float("PERTURB_CHUNK_P_STEP", 0.0025)
...
# Hybrid margin: rank channels & pick the update DIRECTION from a SOFT objective
# (z_true - tau*logsumexp(top-M wrong / tau)) that blends the top-M competing classes, instead of
# committing to the single runner-up. tau/M adapt to gap (how dominant the leading wrong class is)
# and to boundary proximity. The HARD CW margin still decides accept/break/flip — soft only steers.
_SOFT_GRAD = os.getenv("PERTURB_SOFT_GRAD", "1").strip().lower() in {"1", "true", "yes", "on"}
_SOFT_TOP_M = _env_int("PERTURB_SOFT_TOP_M", 5)          # M wrong classes blended away from boundary
_SOFT_TAU = _env_float("PERTURB_SOFT_TAU", 1.0)          # tau when gap < GAP_LO (classes very close)
_SOFT_TAU_MED = _env_float("PERTURB_SOFT_TAU_MED", 0.5)  # tau when GAP_LO <= gap < GAP_HI
_SOFT_GAP_LO = _env_float("PERTURB_SOFT_GAP_LO", 0.5)    # below: classes near-tied -> softest
_SOFT_GAP_HI = _env_float("PERTURB_SOFT_GAP_HI", 1.5)    # above: one class dominates -> hard grad
_SOFT_NEAR_THRESH = _env_float("PERTURB_SOFT_NEAR_THRESH", 0.2)  # |hard margin| below -> near boundary
_SOFT_NEAR_M = _env_int("PERTURB_SOFT_NEAR_M", 3)        # tighter blend near the boundary
_SOFT_NEAR_TAU = _env_float("PERTURB_SOFT_NEAR_TAU", 0.75)
_SOFT_NEAR_SHRINK = _env_float("PERTURB_SOFT_NEAR_SHRINK", 0.5)  # halve the chunk near the boundary
...
def _soft_margin_and_grad(model, x_chw, target_index):
    """Hybrid objective. Returns (hard_margin, grad, near_boundary).

    hard_margin = z_true - max_{j!=true} z_j  — the validator-faithful status (m<0 == flipped); used
        unchanged for accept/break/flip decisions upstream.
    grad        = ∇ of a SOFT objective  z_true - tau*logsumexp(top-M wrong / tau), a smooth blend of
        the top-M competing classes. As channels move, the identity of the single runner-up can swap;
        blending several wrong classes gives a steadier descent direction than committing to top-1.
        tau/M adapt: classes near-tied (small gap) or near the boundary -> softer/tighter blend; one
        class clearly dominant (large gap) -> fall back to the plain hard-margin gradient.
    near_boundary lets the caller shrink the chunk so it doesn't overshoot the flip.
    """
    x = x_chw.detach().clone().requires_grad_(True)
    logits = logits_for_images(model=model, image_bchw=x.unsqueeze(0))[0]
    true_logit = logits[target_index]
    wrong = logits.clone()
    wrong[target_index] = float("-inf")
    k = min(int(logits.numel()) - 1, max(_SOFT_TOP_M, _SOFT_NEAR_M, 2))
    top_vals, _ = torch.topk(wrong, k)
    hard = true_logit - top_vals[0]
    hard_v = float(hard.item())
    gap = float((top_vals[0] - top_vals[1]).item())

    near = hard_v < _SOFT_NEAR_THRESH
    if near:
        m_eff, tau_eff, use_soft = _SOFT_NEAR_M, _SOFT_NEAR_TAU, True
    elif gap < _SOFT_GAP_LO:
        m_eff, tau_eff, use_soft = _SOFT_TOP_M, _SOFT_TAU, True
    elif gap < _SOFT_GAP_HI:
        m_eff, tau_eff, use_soft = _SOFT_TOP_M, _SOFT_TAU_MED, True
    else:
        use_soft = False

    if use_soft:
        m_eff = max(1, min(m_eff, int(top_vals.numel())))
        soft_wrong = tau_eff * torch.logsumexp(top_vals[:m_eff] / tau_eff, dim=0)
        obj = true_logit - soft_wrong
    else:
        obj = hard
    grad = torch.autograd.grad(obj, x)[0]
    return hard_v, grad.detach(), near
...
def _evaluate(model, clean, cand_chw, target_index, device, floor, cap, skip_roundtrip=False,
              envelope=False, log_gap=False):
    ...
    envelope — accept on the worst-case margin over both TF32 settings (one extra forward, CUDA only).
    log_gap — also measure the TF32 drift gap (m_on - m_off) for tuning, without changing the accept
        decision. Free when envelope is on (m_on already computed); else one extra forward (CUDA only).
    tf32_gap in the result is m_on - m_off (>0 means TF32-on is closer to un-flipping), or None.
    """
    ...
    margin, tf32_gap = m_off, None
    if device.type == "cuda" and (envelope or log_gap):
        m_on = _margin_tf32on(model, seen, target_index)
        tf32_gap = m_on - m_off
        if envelope:
            # Worst-case over TF32 off/on, so a flip here survives the validator's unknown TF32 choice.
            margin = max(m_off, m_on)
    flipped = margin < 0.0
    ...
    return {"cand": cand_chw, "linf": linf, "rmse": rmse, "margin": margin, "tf32_gap": tf32_gap,
...
def perturb(
    ...
    def margin_and_grad(x_chw):
        """Hard margin + the ranking/step gradient. With PERTURB_SOFT_GRAD the gradient comes from the
        adaptive soft objective (hard margin still drives accept/break); else the plain hard-margin
        gradient. Third element flags boundary proximity for the per-chunk shrink."""
        if _SOFT_GRAD:
            return _soft_margin_and_grad(model, x_chw, target_index)
        m_, g_ = _margin_and_grad(model, x_chw, target_index)
        return m_, g_, False

    g_t0 = time.time()
    m0, g0, near = margin_and_grad(clean)
    if device.type == "cuda":
    ...
    while time_left() > 2 * t_step:
        if chunks > 0:
            m, g, near = margin_and_grad(cur_float())
        gflat = g.view(-1)
        last_gflat = gflat
        ...
        if near:                                 # near the boundary: smaller chunk, don't overshoot
            b = max(1, int(b * _SOFT_NEAR_SHRINK))
        sel = order[:b]
    ...
    if best_safe is None and changed.any() and time_left() > 1.3 * t_step:
        m_fin, g_fin, _ = margin_and_grad(cur_float())
    ...
    if anchor is not None and anchor["soft"]:
        final = _evaluate(model, clean, anchor["cand"], target_index, device, floor, cap, skip,
                          envelope=envelope_on, log_gap=_LOG_TF32_GAP)
        nz = int(((final["cand"] - clean).abs() > 0.5 * q).sum().item())
        pct = 100.0 * nz / max(1, n)
        gap_str = "" if final["tf32_gap"] is None else f" tf32_gap={final['tf32_gap']:+.5f}"
        logger.info(f"[chunked] flip channels={nz} ({pct:.2f}%) chunks={chunks} b_max={b_max} "
                    ...
                    f"softgrad={'on' if _SOFT_GRAD else 'off'}{gap_str}")
