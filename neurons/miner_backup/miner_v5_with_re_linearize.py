"""
miner_v5_with_re_linearize.py — Strategy 2: Re-linearise at the decision boundary

CHANGE vs v4: After Phase 1 binary search finds the first valid flip (first_flip_n),
bisect on the line x_clean → x_adv to locate x_boundary (the point just before the
flip), recompute the gradient there, and run a second binary search from scratch with
the new gradient ordering.

WHY IT HELPS:
  The linear approximation  margin(x) ≈ m0 + <g, x - x_clean>  is only valid near
  x_clean.  By the time we move to x_adv = x_clean + (n * 1/255) * sparse_signs, the
  true boundary may have rotated considerably.  At x_boundary the gradient g_bnd is
  tangent to the actual decision surface, so the cumulative sum predicts the minimum
  channel count much more accurately.

HOW IT WORKS:
  1. Phase 1 binary search finds (lo, first_flip_n) — best valid candidate is stored
     in `best`.
  2. Boundary bisection: binary search on λ ∈ [0,1] where x(λ) = clean + λ*(adv-clean).
     Use PNG-roundtrip + predict_index (cheap: no grad).  ~6 steps → λ precision 1/64.
  3. x_boundary = x(λ_hi) — the last λ that still flips the label.
  4. Recompute m_bnd, grad_bnd at x_boundary.
  5. Run a fresh binary search (exponential doubling + bisect) with grad_bnd ordering.
     consider() updates `best` automatically if the new result has lower RMSE.

EXPECTED GAIN: 10–40% fewer channels, especially when m0 >> 1.0 (high-confidence images
where the boundary is far from x_clean and the gradient rotates significantly).
"""
...
# Number of bisection steps to find x_boundary.  6 → λ resolution 1/64; 8 → 1/256.
...
def _run_sparse_binary_search(clean, g_flat, k_min, valid_count, sparse_order, g_abs_valid,
    ...
    """
    Shared helper: exponential doubling + binary search on a gradient-sorted sparse perturbation.
    Updates `best` via consider().  Returns the final first_flip_n (or None).
    """
...
def perturb(
    ...
    """Sparse minimum-RMSE L∞ flip with boundary re-linearization (Strategy 2)."""
    ...
    if m0 > 0.0:
        ...
        if valid_count > 0:
            _run_sparse_binary_search(clean, g_flat, k_min, valid_count, sparse_order,
                                      g_abs_valid, m0, consider, time_left, t_png)
    ...
    # =========================== RE-LINEARIZE AT BOUNDARY ================================
    # Only worthwhile if we have a valid candidate AND enough time for bisection + backward
    # + a second binary search.  Budget: bisect_steps * t_png + t_step (backward) +
    # O(log N) * t_png for the second search.
    _bisect_steps = _RELINEARIZE_BISECT_STEPS
    needed = _bisect_steps * t_png + t_step + 4 * t_png
    if best is not None and time_left() > needed:
        x_adv = best["cand"].detach()
        delta_dir = x_adv - clean  # direction vector (already clipped by _make_sparse)

        # Bisect on the line clean → x_adv to find the label-flip boundary
        ...
        # x_boundary is the smallest λ we found that flips the label
        if time_left() > t_step + 2 * t_png:
            ...
            # m_bnd should be ≈ 0 (just past the boundary).  If it's badly negative,
            # the gradient still points inward, which is fine for re-search.
            # If it's very positive somehow, the bisection stalled — skip.
            if m_bnd < 1.0 and time_left() > 2 * t_png:
                g2 = grad_bnd.view(-1)
                c2 = clean.view(-1)
                dir2 = -g2.sign()
                can2 = (((dir2 > 0) & (c2 < 1.0)) | ((dir2 < 0) & (c2 > 0.0)))
                g2_valid = g2.abs().clone()
                g2_valid[~can2] = 0.0
                ord2 = torch.argsort(g2_valid, descending=True)
                vc2 = int((g2_valid > 0).sum().item())

                if vc2 > 0:
                    # Use a small positive margin for the threshold (boundary is ≈ 0)
...
def build_config() -> typing.Any:
    parser = argparse.ArgumentParser(description="Perturb subnet miner (re-linearize at boundary)")
