"""
miner_v5_with_target_class_selection.py — Strategy 1: Target class selection

CHANGE vs v4: Before Phase 1, probe the N closest wrong classes and pick the one
that requires the fewest channels to flip to (lowest estimated sparse cost).

WHY IT HELPS:
  The untargeted gradient follows logit[true] - max_j logit[j].  This gradient points
  toward the overall "hardest" decision boundary, which may not be the nearest one in
  pixel-count space.  A different wrong class t* might lie across a much thinner boundary —
  fewer channels need to move.

HOW IT WORKS:
  1. Forward pass → all logits.  Find top-K wrong classes with smallest logit gap
     margin_t = logit[true] - logit[t]  (smallest gap = closest boundary).
  2. For each candidate t, run one backward pass to get g_t = ∂margin_t/∂x.
  3. Estimate n_t: how many channels (sorted by |g_t|) are needed so their
     cumulative gradient sum crosses the flip threshold margin_t / (k_min/255).
  4. Pick t* = argmin n_t.  Use g_{t*} for Phase 1 binary search.
  5. consider() still accepts any label flip (not just to t*), so a path toward t*
     that accidentally flips to a third class is still a valid win.

EXPECTED GAIN: 20–60% fewer channels when the nearest boundary differs from the
max-logit boundary (common for closely competing classes in EfficientNetV2-L).
"""
...
# How many wrong classes to probe for target selection.
# Each probe costs one backward pass (~t_step). 10 probes ≈ 50ms on GPU.
...
def _targeted_margin_and_grad(model, x_chw, true_index, attack_class):
    """Targeted margin toward a specific wrong class: logit[true] - logit[attack_class]."""
    x = x_chw.detach().clone().requires_grad_(True)
    logits = logits_for_images(model=model, image_bchw=x.unsqueeze(0))[0]
...
def _estimate_sparse_n(grad_flat, clean_flat, margin, k_min):
    """Estimate how many sorted-|grad| channels are needed to reduce margin to 0."""
    ...
    threshold = margin / max(k_min * _Q, 1e-12)
    cumsum = torch.cumsum(g_abs[order], dim=0)
    n_est = min(int((cumsum < threshold).sum().item()) + 1, valid_count)
    return n_est, order, g_abs, valid_count


def _select_best_target(model, clean, true_index, device, k_min, n_probe, time_budget):
    """
    Among the n_probe closest wrong classes, pick the one with the lowest estimated
    sparse channel count.  Returns (best_class, best_grad, best_order, best_g_abs, best_vc).
    Falls back to untargeted gradient if all probes fail or time runs out.
    """
    # One no-grad forward to rank all classes by logit gap
    with torch.no_grad():
        all_logits = logits_for_images(model=model, image_bchw=clean.unsqueeze(0))[0]

    n_classes = all_logits.shape[0]
    # Sort wrong classes by ascending margin (smallest gap = closest boundary)
    candidates = sorted(
    ...
    probe_limit = min(n_probe, len(candidates))

    for margin_t, t in candidates[:probe_limit]:
        if time.time() + 0.001 > time_budget:
            break
        if margin_t <= 0.0:
            # Already adversarial toward t — extremely rare
            continue
        m_t, g_t = _targeted_margin_and_grad(model, clean, true_index, t)
        n_t, order_t, g_abs_t, vc_t = _estimate_sparse_n(g_t.view(-1), clean_flat, m_t, k_min)
        if n_t < best_n:
            best_n = n_t
            best_class = t
            best_grad = g_t
...
def perturb(
    ...
    """Sparse minimum-RMSE L∞ flip with target class selection (Strategy 1)."""
    ...
    # Probe the _TARGET_PROBE_N closest wrong classes and pick the one that (by linear
    # gradient estimate) requires the fewest channels to flip to.  Budget: n_probe * t_step.
    grad_use = grad0          # gradient used by Phase 1 (may be replaced by targeted)
    sparse_order_use = None   # will be set during Phase 1 setup
    g_abs_use = None
    valid_count_use = 0
    ...
    if m0 > 0.0 and time_left() > (_TARGET_PROBE_N + 2) * t_step:
        ...
        if best_tgrad is not None and best_tvc > 0:
            ...
            logger.debug(f"[target_sel] chose class={best_t}, est_n={int((best_tg_abs[best_torder].cumsum(0) < m0 / max(k_min * _Q, 1e-12)).sum().item()) + 1}")
    ...
    if m0 > 0.0:
        ...
        # If target selection didn't provide pre-computed order, compute it now
        if sparse_order_use is None or g_abs_use is None:
            ...
            g_abs_use = g_flat.abs().clone()
            g_abs_use[~can_move] = 0.0
            sparse_order_use = torch.argsort(g_abs_use, descending=True)
            valid_count_use = int((g_abs_use > 0).sum().item())

        sparse_order = sparse_order_use
        g_abs_valid = g_abs_use
        valid_count = valid_count_use

        if valid_count > 0:
            # Use the targeted class margin for the threshold if available
            m_use = m0  # untargeted margin — always positive when we get here
...
def build_config() -> typing.Any:
    parser = argparse.ArgumentParser(description="Perturb subnet miner (target class selection)")
