"""
miner_v5_with_1_2.py — Strategies 1+2: Target class selection + Re-linearise at boundary
...
CHANGES vs v4:
  Strategy 1 — Target class selection (before Phase 1):
    ...
    flip to (lowest estimated sparse cost via cumulative gradient sum), and use its gradient
    for the Phase 1 binary search.
  ...
  Strategy 2 — Re-linearise at boundary (after Phase 1):
    ...
    exact decision boundary, recompute the gradient at that point, and run a second
    binary search with the new gradient ordering.  The boundary gradient is tangent to
    the true decision surface, typically giving a significantly sparser second solution.

Together: Strategy 1 directs the search toward the easiest-to-flip class, and Strategy 2
then tightens the result by correcting the linear approximation.  Expected combined
improvement: 30–80% fewer channels vs v4.
"""
...
def _build_sparse_order(grad_flat, clean_flat):
    """Compute gradient-sorted order and valid count for sparse perturbation."""
...
def perturb(
    ...
    """Sparse minimum-RMSE L∞ flip with target class selection + re-linearization (Strategies 1+2)."""
    ...
    # PHASE 1 — sparse top-k binary search with selected gradient
    if m0 > 0.0:
    ...
    # STRATEGY 2 — Re-linearise at boundary
...
def build_config() -> typing.Any:
    parser = argparse.ArgumentParser(description="Perturb subnet miner (strategies 1+2)")
